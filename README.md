...
| exercise 7 | API interception | 1. Intercept the POST request 2. Assert the request body with title, body an userId 3. Continue the intercepted request and assert the response 4. Alias the intercept and send the POST manually 5. Assert the response | Show message that the response status is 201 (created) |
---
